...
steady = modelsteady_GHH(delta, alpha, bbeta, hss, gamma, nu)
...
fprintf(fid, '\n');
fprintf(fid, 'parameters bbeta psi alpha delta gamma abar rho_a sigma_a nu; \n');
fprintf(fid, '\n');
...
fprintf(fid, 'model;\n');
fprintf(fid, '1/(exp(c) - psi *(exp(h)^(1+nu)/(1+nu))) = exp(lambda);\n');
fprintf(fid, 'gamma = (bbeta/gamma) * (exp(lambda(+1))/exp(lambda)) *((1-alpha) * exp(a(+1)) * exp(k)^(-alpha) * exp(h)^alpha + (1-delta));\n');
fprintf(fid, 'psi * exp(h)^nu = alpha * exp(a) * exp(k(-1))^(1-alpha)*exp(h)^(alpha-1);\n');
